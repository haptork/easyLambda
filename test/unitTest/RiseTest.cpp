...
void RiseTest(int argc, char* argv[]) {
  RiseBasicTest();
}
...
void RiseBasicTest() {
  ...
  auto r1 = make_shared<Rise<decltype(f1)>>(ProcReq{1}, std::move(f1), nullptr);
  ...
  auto r2 = make_shared<Rise<decltype(f2)>>(ProcReq{1}, std::forward<decltype(f2)>(f2), nullptr);
  r2->par(pr);
  r2->next(ret, r2);
  r2->pull();
  assert(count == 0);
  ...
  auto r3 = make_shared<Rise<decltype(f3)>>(ProcReq{1}, std::move(f3), nullptr);
  r3->par(pr);
  r3->next(ret, r3);
  r3->pull();
  assert(count == 2);
  ...
  auto r4 = make_shared<Rise<decltype(f4)>>(ProcReq{1}, std::move(f4), nullptr);
  r4->par(pr);
  r4->next(ret, r4);
  r4->pull();
  assert(count == 0);
